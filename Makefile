...
CLEAN_GOOGLE_CLIENT_ID := $(or $(subst ",,$(GOOGLE_CLIENT_ID)),716595821548-mmgp3ivk20bboapvlsru7kh08dp0n7c7.apps.googleusercontent.com)
CLEAN_GCIP_API_KEY := $(subst ",,$(GCIP_API_KEY))
CLEAN_GCIP_AUTH_DOMAIN := $(or $(subst ",,$(GCIP_AUTH_DOMAIN)),$(CLEAN_PROJECT_ID).firebaseapp.com)
CLEAN_ALLOWED_DOMAINS := $(subst ",,$(ALLOWED_DOMAINS))
CLEAN_ALLOWED_USERS := $(subst ",,$(ALLOWED_USERS))
CLEAN_ENABLE_DEMO_AUTH := $(or $(subst ",,$(ENABLE_DEMO_AUTH)),false)
CLEAN_USE_FIRESTORE := $(or $(subst ",,$(USE_FIRESTORE)),true)
CLEAN_FIRESTORE_DATABASE := $(or $(subst ",,$(FIRESTORE_DATABASE)),tts-jobs)
CLEAN_FIRESTORE_COLLECTION := $(or $(subst ",,$(FIRESTORE_COLLECTION)),tts_jobs)
...
deploy: ## Deploy application directly to Google Cloud Run via Cloud Build
	...
	gcloud run deploy $(SERVICE_NAME) \
		...
		--set-env-vars '^##^GCP_PROJECT_ID=$(CLEAN_PROJECT_ID)##GCP_LOCATION=$(CLEAN_LOCATION)##GCS_BUCKET_NAME=$(CLEAN_BUCKET)##GEMINI_VOICE_MODEL=$(CLEAN_VOICE_MODEL)##GEMINI_JUDGE_MODEL=$(CLEAN_JUDGE_MODEL)##GEMINI_JUDGE_LOCATION=$(CLEAN_JUDGE_LOCATION)##DEFAULT_VOICE_PERSONA=$(CLEAN_PERSONA)##AUDIO_BITRATE=$(CLEAN_BITRATE)##GOOGLE_CLIENT_ID=$(CLEAN_GOOGLE_CLIENT_ID)##GCIP_API_KEY=$(CLEAN_GCIP_API_KEY)##GCIP_AUTH_DOMAIN=$(CLEAN_GCIP_AUTH_DOMAIN)##ALLOWED_DOMAINS=$(CLEAN_ALLOWED_DOMAINS)##ALLOWED_USERS=$(CLEAN_ALLOWED_USERS)##ENABLE_DEMO_AUTH=$(CLEAN_ENABLE_DEMO_AUTH)##USE_FIRESTORE=$(CLEAN_USE_FIRESTORE)##FIRESTORE_DATABASE=$(CLEAN_FIRESTORE_DATABASE)##FIRESTORE_COLLECTION=$(CLEAN_FIRESTORE_COLLECTION)'

.PHONY: migrate-data
migrate-data: ## Migrate local SQLite jobs to Cloud Firestore Native database
	@echo "📦 Migrating local SQLite jobs to Google Cloud Firestore ($(CLEAN_FIRESTORE_DATABASE))..."
